...
class MapTRv2Head(DETRHead):
    ...
    def __init__(self,
        ...
        super(MapTRv2Head, self).__init__(
            *args, transformer=transformer, positional_encoding=positional_encoding, **kwargs)
        if self.modality is 'sem_map':
    ...
    def _init_layers(self):
        ...
        if not self.as_two_stage:
            if 'BEVFormerEncoder' in self.bev_encoder_type and self.modality != 'sem_map':
    ...
    @force_fp32(apply_to=('mlvl_feats', 'prev_bev', 'sem_map_feat'))
    def forward(self, mlvl_feats, lidar_feat, sem_map_feat, img_metas, prev_bev=None,  only_bev=False):
        """Forward function.
        Args:
        ...
        if mlvl_feats is not None:
            ...
            bs = sem_map_feat.size(0)
            dtype = sem_map_feat.dtype
        # import ipdb;ipdb.set_trace()
        if self.query_embed_type == 'all_pts':
        ...
        if self.bev_embedding is not None and mlvl_feats is not None:
