...
class MapTRv2(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        if self.modality == "fusion" or self.modality == "sem_map":
            self.sem_map_backbone = builder.build_backbone(sem_map_backbone)
            sem_pretrained = pretrained.get('sem', None)
            warnings.warn('DeprecationWarning: pretrained is a deprecated \
                    key, please consider using init_cfg : SEM LOADED!')
            self.sem_map_backbone.init_cfg = dict(
                    type='Pretrained', checkpoint=sem_pretrained)
            self.sem_map_neck = builder.build_neck(sem_map_neck)
    ...
    @force_fp32(apply_to=('sem_map'))
    def extract_sem_map_feat(self, sem_map):
        # sem_map = sem_map.permute(0,3,1,2).contiguous()
        sem_map_feat = self.sem_map_backbone(sem_map)
        # for idx, feat in enumerate(sem_map_feat):
        #     print(idx, feat.shape)
        # breakpoint()
        sem_map_feat = self.sem_map_neck(sem_map_feat)
        return sem_map_feat[0]
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(
            pts_feats, lidar_feat, sem_map_feat, img_metas, prev_bev)
    ...
    @force_fp32(apply_to=('img','points','prev_bev','sem_map'))
    def forward_train(self,
        ...
        sem_map_feat = None
        prev_bev = None
        if self.modality == 'fusion' and points is not None:
            lidar_feat = self.extract_lidar_feat(points)
        
        if (self.modality == 'fusion' or self.modality == 'vision') and img is not None:
        ...
        if (self.modality == 'sem_map' or self.modality == 'fusion') and sem_map is not None:
            sem_map_feat = self.extract_sem_map_feat(sem_map)
        
        losses = dict()
        losses_pts = self.forward_pts_train(img_feats, lidar_feat, sem_map_feat, gt_bboxes_3d,
    ...
    def forward_test(self, img_metas, img=None,points=None, sem_map = None, **kwargs):
        ...
        sem_map = [sem_map] if sem_map is None else sem_map
        img_metas = [img_metas] if img_metas is None else img_metas
        if (self.modality == 'fusion' or self.modality == 'vision') and img is not None:
        ...
        new_prev_bev, bbox_results = self.simple_test(
            img_metas[0], img[0], points[0], sem_map, prev_bev=self.prev_frame_info['prev_bev'], **kwargs)
    ...
    def pred2result(self, bboxes, scores, labels, pts, attrs=None):
        ...
        return result_dict
    def simple_test_pts(self, x, lidar_feat, sem_map_feat, img_metas, prev_bev=None, rescale=False):
        """Test function"""
        outs = self.pts_bbox_head(x, lidar_feat, sem_map_feat, img_metas, prev_bev=prev_bev)
    ...
    def simple_test(self, img_metas, img=None, points=None, sem_map=None, prev_bev=None, rescale=False, **kwargs):
        ...
        if self.modality =='fusion' and points is not None:
            lidar_feat = self.extract_lidar_feat(points)
        if (self.modality == 'vision' or self.modality == 'fusion') and img is not None:
            img_feats = self.extract_feat(img=img, img_metas=img_metas)
            bbox_list = [dict() for i in range(len(img_metas))]
        if (self.modality == 'sem_map' or self.modality == 'fusion') and sem_map is not None:
            sem_map_feat = self.extract_sem_map_feat(sem_map)
            bbox_list = [dict() for i in range(len(sem_map))]
        new_prev_bev, bbox_pts = self.simple_test_pts(
            img_feats, lidar_feat, sem_map_feat, img_metas, prev_bev, rescale=rescale)
